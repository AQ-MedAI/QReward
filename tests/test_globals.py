...
class TestOverloadExceptions:
    ...
    def test_overload_exceptions_is_frozenset(self):
        ...
        assert isinstance(OVERLOAD_EXCEPTIONS, frozenset), (
        ...
        for exception in expected_exceptions:
            assert exception in OVERLOAD_EXCEPTIONS, (
...
class TestLibraryOverloadExceptionsMapping:
    ...
    def test_library_overload_exceptions_mapping_structure(self):
        ...
        assert isinstance(LIBRARY_OVERLOAD_EXCEPTIONS_MAPPING, dict), (
        ...
        for key in expected_keys:
            assert key in LIBRARY_OVERLOAD_EXCEPTIONS_MAPPING, (
    ...
    def test_library_mapping_values_are_nonempty(self):
        """验证每个库的异常集合非空"""
        for library_name, exceptions in (
            LIBRARY_OVERLOAD_EXCEPTIONS_MAPPING.items()
        ):
            assert len(exceptions) > 0, (
                f"{library_name} 的异常集合不应该为空"
            )
...
class TestSystemOverloadIndicators:
    ...
    def test_system_overload_indicators_is_frozenset(self):
        ...
        assert isinstance(SYSTEM_OVERLOAD_INDICATORS, frozenset), (
        ...
        for errno in expected_errno:
            assert errno in SYSTEM_OVERLOAD_INDICATORS, (
...
class TestOverloadKeywords:
    ...
    def test_overload_keywords_is_frozenset(self):
        ...
        'timeout', 'rate limit'"""
        assert isinstance(OVERLOAD_KEYWORDS, frozenset), (
        ...
        for keyword in expected_keywords:
            assert keyword in OVERLOAD_KEYWORDS, (
