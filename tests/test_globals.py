...
class TestLibraryOverloadExceptionsMapping:
    ...
    def test_library_overload_exceptions_mapping_structure(self):
        """验证 LIBRARY_OVERLOAD_EXCEPTIONS_MAPPING 是 dict，包含五个 key，每个 value 是 frozenset"""
        assert isinstance(
        ...
        for key, value in LIBRARY_OVERLOAD_EXCEPTIONS_MAPPING.items():
            assert isinstance(
                value, frozenset
            ), f"LIBRARY_OVERLOAD_EXCEPTIONS_MAPPING['{key}'] 应该是 frozenset 类型"
...
class TestOverloadKeywords:
    ...
    def test_overload_keywords_is_frozenset(self):
        """验证 OVERLOAD_KEYWORDS 是 frozenset 且包含关键词如 'overload', 'timeout', 'rate limit'"""
        assert isinstance(
